...
function RadioPage() {
    ...
    const initTrack = RADIO_EP_INFO.length-1;
    // states
    const [trackIndex, setTrackIndex] = useState(initTrack);
    const [currentTrack, setCurrentTrack] = useState(RADIO_EP_INFO[trackIndex]);
    ...
    const handleNext = () => {
      ...
      if (trackIndex >= RADIO_EP_INFO.length - 1) {
        setTrackIndex(0);
        ...
        setCurrentTrack(RADIO_EP_INFO[trackIndex + 1]);
